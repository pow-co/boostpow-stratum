...
import { private_key_wallet } from "./bitcoin";
...
export async function start() {
  ...
  if (args.length !== 3) {
    ...
    console.log(
      "the first argument should be a WIF private key that will be used as a wallet."
    );
    return;
  } else {
    try {
  ...
  await stratum.start(
    job_manager(await listJobs(), private_key_wallet(key), 10)
  );

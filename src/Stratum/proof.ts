...
export function work_puzzle(
  ...
  n: notify_params,
  version_mask?: string
): boostpow.work.Puzzle | undefined {
  if (
    !Extranonce.valid(en) ||
    !NotifyParams.valid(n) ||
...
export function prove(
  ...
  if (!valid_share || right_params || nonce_length || ver_mask_1 || ver_mask_2)
    return;
...
export class Proof {
  ...
  constructor(en: extranonce, n: notify_params, x: share, mask?: string) {
    this.jobID = n[0];

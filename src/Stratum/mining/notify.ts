...
export class NotifyParams {
  static valid(params: notify_params): boolean {
    let is_hex = function (hex) {
      return (
        typeof hex === "string" &&
        /^(([0-9a-f][0-9a-f])*)|(([0-9A-F][0-9A-F])*)$/.test(hex)
      );
    };
    ...
    if (
      !(
        ...
        SessionID.valid(params[5]) &&
        SessionID.valid(params[6]) &&
        SessionID.valid(params[7]) &&
        typeof params[8] === "boolean"
      )
    ) {
      return false;
    }
  ...
  static nbits(p: notify_params): boostpow.Difficulty {
    if (this.valid(p)) {
      return boostpow.Difficulty.fromBits(
        boostpow.UInt32Little.fromHex(p[6]).number
      );
    }
...
export class Notify extends Notification {
  ...
  static read(message: JSONValue): notify | undefined {
    ...
    if (
      !n ||
      n["method"] !== "mining.notify" ||
      n.params.length != 9 ||
      typeof n.params[0] !== "string" ||
      typeof n.params[1] !== "string" ||
      typeof n.params[2] !== "string" ||
      typeof n.params[3] !== "string" ||
      !Array.isArray(n.params[4]) ||
      typeof n.params[5] !== "string" ||
      typeof n.params[6] !== "string" ||
      typeof n.params[7] !== "string" ||
      typeof n.params[8] !== "boolean"
    )
      return;

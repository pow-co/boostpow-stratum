...
export type extension_requests = {
...
export class Extensions {
  ...
  // to iterate over.
  static extension_requests(
    p: configure_params
  ): extension_requests | undefined {
    var requests = {};
...
export class ExtensionVersionRolling {
  static valid_params = function (r: {
    mask: string;
    "min-bit-count": number;
  }): boolean {
    return (
      SessionID.valid(r["mask"]) &&
      Number.isInteger(r["min-bit-count"]) &&
      r["min-bit-count"] >= 0
    );
  };
...
export class ConfigureRequest {
  ...
  static make(id: message_id, q: extension_requests): configure_request {
    return {
      ...
      params: Extensions.configure_request_params(q),
    };
  }
}
...
export class ConfigureResponse {
  ...
  static make(
    ...
    return {
      id: id,
      err: err ? err : null,
      result: Extensions.configure_response_result(s),
    };
  }
}

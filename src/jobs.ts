...
export class BoostJob {
  constructor(public jobID: number, public puzzle: boostpow.Puzzle) {}
  ...
  complete(
    solution: boostpow.work.Solution,
    our_address: bsv.Address,
    satsPerByte: number
  ): bsv.Transaction {
    let incomplete_tx_length = 0;
    let estimated_size =
      incomplete_tx_length + this.estimated_locking_script_size();

    let output_value =
      this.puzzle.output.value - Math.ceil(estimated_size * satsPerByte);

    const tx = new boostpow.bsv.Transaction();
    tx.addOutput(
      new boostpow.bsv.Transaction.Output({
        script: boostpow.bsv.Script(
          new boostpow.bsv.Address(our_address.toString())
        ),
...
export interface HashpowerEstimate {
  hashpower: number;
  certainty: number;
}
...
export function job_manager(
  ...
  let add: (o: boostpow.Output) => void = (o: boostpow.Output) => {
    ...
    jobs[boost_output_index(o)] = new BoostJob(
      nextJobID,
  ...
  let select: (boolean, HashpowerEstimate, number) => BoostJob | undefined = (
    ...
    for (let index of Object.keys(jobs)) {
      ...
      if (!version_rolling && j.puzzle.output.script.scriptVersion > 1)
        continue;
      ...
      if (j.puzzle.output.script.scriptVersion == 1 && difficulty > 4000000000)
        continue;
  ...
  let complete = async (
    j: BoostJob,
    x: boostpow.work.Solution
  ): Promise<boolean> => {
    return network.broadcast(
      j.complete(x, keys.nextReceive(), await network.satsPerByte())
    );
  };

...
export type HandleExtension = (requested: ExtensionParameters) => {
  reply: extension_result;
  keep?: ExtensionParameters;
};
...
export function versionRollingHandler(mask: number): HandleExtension {
  return (requested: ExtensionParameters) => {
    if (Object.keys(requested).length != 2)
    ...
    if (
      !requested_mask ||
      !minBitCount ||
      typeof requested_mask !== "string" ||
      !SessionID.valid(requested_mask) ||
      typeof minBitCount !== "number"
    )
      return { reply: ["invalid parameters", {}] };

    const requested_mask_number =
      boostpow.Int32Little.fromHex(requested_mask).number;
    ...
    if (bit_count < minBitCount)
      return { reply: ["could not satisfy min-bit-count", {}] };
...
export function minimumDifficultyHandler(): HandleExtension {
  return (requested: ExtensionParameters) => {
    if (Object.keys(requested).length != 1)
      return { reply: ["invalid parameters", {}] };
...
export let extensionHandlers: ExtensionHandlers = {
  version_rolling: versionRollingHandler(

...
describe("Jobs", () => {
  it("Redeem script", () => {
    ...
    const dataHex = new Buffer("this is more additionalData", "ascii").toString(
      "hex"
    );
    ...
    let txid = boostpow.Digest32.fromHex(
      "abcdef00112233445566778899abcdefabcdef00112233445566778899abcdef"
    );
    let vout = 2;
    ...
    let solutionV1 = new boostpow.work.Solution(
      time,
      extraNonce1,
      extraNonce2,
      nonceV1
    ...
    job.complete(
      solutionV1,
      new bsv.Address().fromString("1Hh2ZrptvqCRRN2DztggtAnSyMqfc2s289"),
      1
    );
  });
});

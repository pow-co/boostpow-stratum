...
describe("Stratum Handlers Client -> Server -> Client", () => {
  ...
  let txid = boostpow.Digest32.fromHex(
    "abcdef0a0b0c0d0e0f1122330102030405060708090a0b0c"
  );
  ...
  it("mining.subscribe should return a subscribe response and a new job using the unexteded protocol", async () => {
    ...
    let send = stratum(
      server_session(jobs.subscribe, { canSubmitWithoutAuthorization: true })
    )(dummy.connection);
    ...
    expect(
      (!!SetDifficulty.read(notification1) && !!Notify.read(notification2)) ||
        (!!SetDifficulty.read(notification2) && !!Notify.read(notification1))
    ).to.equal(true);
  });
  ...
  it("mining.configure sends an error if it can't support the right min bit count for version_rolling", async () => {
    ...
    expect(response.result["version_rolling"]).to.be.eql(
      "could not satisfy min-bit-count"
    );
  });
  ...
  let extra_nonce_2_v1: boostpow.Bytes =
    boostpow.Bytes.fromHex("0000000300000003");
  let extra_nonce_2_v2: boostpow.Bytes = boostpow.Bytes.fromHex(
  ...
  describe("mining.submit original protocol tests", async () => {
    ...
    it("mining.submit original protocol fails on unknown job share", async () => {
      ...
      let unknown_job_share = Share.make(
        worker_name,
        "invalid",
        time_now,
        nonce_v1,
    ...
    it("mining.submit original protocol fails on version mask job share", async () => {
      let version_mask_share = Share.make(
        worker_name,
        np[0],
        time_now,
        nonce_v1,
  ...
  describe("mining.submit extended protocol", async () => {
    let jobs;
    let dummy;
    let send;
    let version_rolling_result;
    let version_mask;
    let valid_share;
    let invalid_share;
    let no_version_mask_share;
    let valid_proof;
    let invalid_proof;
    ...
    beforeEach(async (done) => {
      ...
      send({
        ...
        params: Extensions.configure_request_params({
          subscribe_extranonce: {},
          version_rolling: {
            mask: boostpow.Int32Little.fromNumber(
      ...
      version_rolling_result = Extensions.extension_results(
        Response.read(dummy.end.read()).result
      ).version_rolling;
      version_mask = <string>version_rolling_result[1]["mask"];
      send({
        id: 3,
        method: "mining.subscribe",
        params: [worker_name, extra_nonce_1.hex],
      ...
      no_version_mask_share = Share.make(
        worker_name,
        np[0],
        time_now,
        nonce_v1,
      ...
      valid_proof = new Proof(en, np, valid_share, version_mask);
      invalid_proof = new Proof(en, np, invalid_share, version_mask);
      done();
    });

    it("Sets version masks correctly", async () => {
      expect(version_rolling_result[0]).to.equal(true);
      expect(version_mask).to.not.equal(undefined);
    });

    it("Sets proofs correctly", async () => {
      expect(valid_proof.valid()).to.equal(true);
      expect(invalid_proof.valid()).to.equal(false);
    });

    it("mining.submit extended protocol fails on invalid share", async () => {
      {
        send({
          id: 4,
          method: "mining.submit",
    ...
    it("mining.submit extended protocol fails on no_version share", async () => {
      {
        send({
          id: 5,
          method: "mining.submit",
    ...
    it("mining.submit extended protocol succeeds on valid share", async () => {
      {
        send({
          id: 6,
          method: "mining.submit",
